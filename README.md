...
v1.4.0までの全機能とデータを維持したまま、ジュリアスの台詞内容に応じて6種類の新しい立ち絵を使い分ける更新版です。

## v1.4.1の変更

- 新立ち絵 `neutral / smile / focused / concerned / blushing / happy` の6種類を追加
- 既存台詞本文とカテゴリ別抽選、直近台詞の重複回避を維持し、台詞の意味に合わせてmoodだけを再割当て
- 通常・軽い称賛・集中を中心にし、心配は健康や休息、照れと満面の笑顔は明確な特別場面に限定
- 未知のmoodは `neutral` へフォールバックし、6画像を起動時にプリロード
- 新しいバストアップ構図に合わせ、PC・スマホとも画像の拡大率を抑えて枠内に自然な余白を確保
- 新6画像をService WorkerのApp Shellへ追加してオフライン表示に対応
- APP_VERSIONとService Workerキャッシュをv1.4.1へ更新
- schema 11、localStorage、JSON、Firebase、Cloud Sync、CLOUD DATA SIZE、競合防止を変更しない
...
本番データへテスト記録を混ぜないよう、未ログインのローカル確認環境で行ってください。

1. 通常起動がneutral、小さな一歩や通常の完了がneutralまたはsmileになることを確認
2. 作業開始や明確な指示がfocusedになることを確認
3. 深夜起動、睡眠不足、長時間作業・運動の休息案内がconcernedになることを確認
4. 明確に好意が漏れる台詞だけがblushingになることを確認
5. 週間運動目標達成や大きな完了だけがhappyになることを確認
6. 開発者ツールで `setJuliusPortrait('neutral')` から6種類を順に指定し、PC・スマホ幅で顔や頭部が切れず、表示枠が動かないことを確認
7. 未知のmoodを指定するとneutralへ戻ることを確認
8. オフライン再起動でも6画像が表示されることを確認

自動検証の詳細は `VERIFICATION_v1.4.1.md` に記載しています。
...
v1.4.0実装時の自動検証内容は、v1.4.1の回帰検証へ引き継いでいます。
...
1. PCでv1.4.1を開き、表示と既存データを確認してJSONを保存
...
1. 現在のJSONバックアップと、公開中v1.4.0一式のコピーを保管
...
5. Pagesの更新完了後、PCで公開URLを開き `v1.4.1 CLOUD SYNC` 表記を確認
